    ...
    public class TrajectoryArc : MonoBehaviour
    {
...
#if (UNITY_5_4)
			pointerLineRenderer.SetColors( pointerColor, pointerColor );
#else
			pointerLineRenderer.startColor = pointerColor;
			pointerLineRenderer.endColor = pointerColor;
#endif
        ...
        void Update()
        {
            if (leftHand == null) {
                ...
                RaycastHit hitInfo;
                Vector3 arcVelocity = leftHand.transform.forward * 20f;

                SetArcData( trajectoryPoint.position, arcVelocity, true, false );
                DrawArc(out hitInfo);
            }
        }
